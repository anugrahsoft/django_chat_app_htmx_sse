...
async def room_chat_sse(request, room_name):
    ...
    # CORRECTED LOGIC
    last_message_id = 0
    # Prioritize the Last-Event-ID header for reconnections
    if 'HTTP_LAST_EVENT_ID' in request.META:
        last_message_id = int(request.META['HTTP_LAST_EVENT_ID'])
    # Use the query param for the initial connection
    elif request.GET.get('last_id') and request.GET.get('last_id').isdigit():
        last_message_id = int(request.GET.get('last_id'))
