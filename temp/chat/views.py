...
@sync_to_async
def typing_status(request, conversation_type, identifier):
    user = request.user
    is_typing = request.POST.get('is_typing') == 'true'
    
    if is_typing:
        user.last_typing_timestamp = datetime.now()
    else:
        user.last_typing_timestamp = None
...
async def room_chat_sse(request, room_name):
    ...
    async def event_stream(room, last_message_id):
        ...
        typing_users = await sync_to_async(list)(
            User.objects.filter(
                last_typing_timestamp__gte=datetime.now() - timedelta(seconds=2)
            ).exclude(username=request.user.username)
        )
        
        if typing_users:
            typing_message = ", ".join([user.username for user in typing_users])
            yield f"event: typing-indicator\ndata: <small>{typing_message} is typing...</small>\n\n".encode('utf-8')
...
async def chat_sse(request, recipient_username):
    ...
    async def event_stream(user, recipient, last_message_id):
        ...
        if recipient.last_typing_timestamp and recipient.last_typing_timestamp >= datetime.now() - timedelta(seconds=2):
            yield f"event: typing-indicator\ndata: <small>{recipient.username} is typing...</small>\n\n".encode('utf-8')
        else:
            yield "event: typing-indicator\ndata: \n\n".encode('utf-8')
