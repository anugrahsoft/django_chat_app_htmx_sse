...
from asgiref.sync import sync_to_async
from .message_queue import chat_queue
...
@async_login_required
async def update_last_seen(request):
    user = await request.auser()
    if user.is_authenticated:
        await sync_to_async(User.objects.filter(id=user.id).update)(last_seen=timezone.now())
    return HttpResponse("")
...
async def room_chat_sse(request, room_name):
    ...
    async def event_stream(room, last_message_id):
        while True:
            # Poll for new messages (as before)
            ...
            # --- Status update logic (inside the loop) ---
            online_threshold = timezone.now() - timedelta(seconds=60)
            
            # Re-fetch all users to get latest last_seen timestamp
            all_users = await sync_to_async(list)(
                User.objects.filter(last_seen__gte=online_threshold)
            )
            
            online_usernames = [u.username for u in all_users]
            
            yield f"event: status-update\n"
            yield f"data: {online_usernames}\n\n".encode('utf-8')
            # ---------------------------
...
async def chat_sse(request, recipient_username):
    ...
    async def event_stream(user, recipient, last_message_id):
        while True:
            # Construct the QuerySet and then access its query attribute
            qs = Message.objects.filter(
                (models.Q(user=user, recipient=recipient) | models.Q(user=recipient, recipient=user))
            ).filter(id__gt=last_message_id)
            
            # This is the correct way to get the query
            #print(str(qs.query))


            # Poll for new messages
            new_messages = await sync_to_async(list)(qs.select_related('user', 'recipient'))
            '''
            new_messages = await sync_to_async(list)(
                Message.objects.filter(
                    (models.Q(user=user, recipient=recipient) | models.Q(user=recipient, recipient=user))
            ...
            status = 'offline'
            try:
                # Re-fetch the recipient user object from the database
                updated_recipient = await sync_to_async(User.objects.get)(username=recipient.username)
                
                online_threshold = timezone.now() - timedelta(seconds=60)
                is_recipient_online = updated_recipient.last_seen and updated_recipient.last_seen >= online_threshold
                
                status = 'online' if is_recipient_online else 'offline'
...
def post_message(request):
    """Receives and broadcasts a new chat message."""
    if request.method == "POST":
        message = request.POST.get("message")
        if message:
            # The 'put_nowait' method is called correctly on the async queue
